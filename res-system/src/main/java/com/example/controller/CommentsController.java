...
public class CommentsController {
    ...
    public ResponseEntity<?> getTags(@PathVariable String analysisId) {
        String sql = "SELECT userid, analysis_id, frame_index, tag, memo, timestamp_sec " +
        ...
        for (Object[] row : results) {
            ...
            map.put("analysisId", row[1]);
            map.put("frameIndex", row[2]);
            map.put("tag", row[3]);
            map.put("memo", row[4]);
            map.put("timestamp", row[5].toString());  // Timestamp → String
            response.add(map);
        }
    ...
    public ResponseEntity<?> deleteTag(@PathVariable String analysisId) {
